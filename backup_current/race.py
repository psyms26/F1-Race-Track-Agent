...
class Race:
    ...
    def _compute_grip(self) -> float:
        """Grip = base[weather][type] × (1 - wear_penalty × wear) × recovery."""
    ...
    def _apply_throttle(
        ...
        """Convert throttle command into a speed change."""
        # Maximum achievable speed limited by both top speed and tyre compound
        ...
        if throttle >= 0.0:
            ...
            if self.speed < target:
                ...
                # Slight passive deceleration when target below current speed
                self.speed = max(target, self.speed - config.ACCEL_RATE * 0.5 * dt)
        else:
            # Negative throttle = brake
    ...
    def _apply_steering(self, steering: float, max_safe: float, dt: float) -> None:
        """Update lateral position from steering input + corner-overshoot drift.
        Steering authority scales with speed — a stationary car cannot turn."""
        # Direct steering response, scaled by speed (no steering at standstill)
        ...
        # Corner-overshoot drift: if exceeding safe speed in a corner, get pushed
        # outward (away from corner direction)
    ...
    def _handle_offtrack_transition(
        ...
        """One-shot wear on entry; counted excursion on entry; recovery on exit."""
        if is_off and not self.was_off_track:
            ...
            if self.offtrack_count > config.OFFTRACK_FREE_EXCURSIONS:
                # Each excursion beyond the free quota costs configured seconds
        ...
        if not is_off and self.was_off_track:
            # Just returned to track → start grip recovery
    ...
    def _handle_lap_crossing(self) -> float:
        """Called when s wraps. Records lap, updates weather, services pit."""
        # Record lap time
        ...
        # Servicing the pit happens BEFORE incrementing lap counter so the new
        # tyres take effect from the next lap onwards.
    ...
    def _service_pit(self) -> None:
        """Apply pit-stop time cost and swap tyres."""
        # Time cost: traversing pit lane at the limit + tyre change time
    ...
    def _finish_race(self) -> float:
        """Race completed — apply post-race compliance checks."""
    ...
    def _observation(self) -> np.ndarray:
        """Build the 19-dim state vector used by RL (and accessible to others)."""
        # 1 each: speed, wear, fuel, laps_remaining, has_pitted
...
# =============================================================================
# Sanity check — drive a dummy "always full throttle" agent for one race
# =============================================================================
...
if __name__ == "__main__":
    ...
    # Simple driver: full throttle on straights, partial through corners
